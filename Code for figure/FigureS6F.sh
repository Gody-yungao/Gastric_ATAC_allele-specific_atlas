...
anno_df_caOCR=subset(anno_df,anno_df$V4 %in% caOCR$V4)
dir.create("/data1/gy/ATACseq_RWAS/caQTL_STITCH/caQTL_model_result/TMM_exp/15.sex_age_Hp_PC1_5_peer1_7/200kb_nominal/chipseeker_anno")
fwrite(anno_df_caOCR,
       "/data1/gy/ATACseq_RWAS/caQTL_STITCH/caQTL_model_result/TMM_exp/15.sex_age_Hp_PC1_5_peer1_7/200kb_nominal/chipseeker_anno/95sample_caQTL_200kb_nominal_result.withFDR.FDR0.1.UCSC_hg19_known_gene.anno_by_chipseeker.txt",
       sep="\t",quote=F,col.names=T,row.names=F)
##noncaOCR
anno_df_noncaOCR=subset(anno_df,!(anno_df$V4 %in% caOCR$V4))

##
table(anno_df_caOCR$region)
#   3'UTR    5'UTR   Distal     Exon   Intron Promoter 
#     404      493     4302      863     4631     2972
table(anno_df_noncaOCR$region)
#   3'UTR    5'UTR   Distal     Exon   Intron Promoter 
#    3436     3733    25476     6690    30710    25477

##
data_caOCR=as.data.frame(table(anno_df_caOCR$region))
data_caOCR$class="caOCR"
data_noncaOCR=as.data.frame(table(anno_df_noncaOCR$region))
data_noncaOCR$class="non-caOCR"
##
data_caOCR <- data_caOCR %>%
  group_by(class) %>%
  mutate(pct = Freq / sum(Freq) * 100)
data_noncaOCR <- data_noncaOCR %>%
  group_by(class) %>%
  mutate(pct = Freq / sum(Freq) * 100)
data=rbind(data_caOCR,data_noncaOCR)
...
#
total_caOCR    <- sum(wide$caOCR)  
total_noncaOCR <- sum(wide$noncaOCR)  
...
res <- wide %>%  
  ...
  mutate(  
    ...
    tab = list(matrix(  
      c(caOCR,  
        total_caOCR - caOCR,  
        noncaOCR,  
        total_noncaOCR - noncaOCR),  
      nrow = 2,  
      byrow = TRUE,  
      dimnames = list(  
        sample = c("caOCR", "non-caOCR"),  
  ...
  ungroup() %>%  
  select(Var1, caOCR, noncaOCR, OR, p.value,CI_Lower,CI_Upper)  
...
fwrite(res,
       "/data1/gy/ATACseq_RWAS/caQTL_STITCH/caQTL_model_result/TMM_exp/15.sex_age_Hp_PC1_5_peer1_7/200kb_nominal/chipseeker_anno/95sample_caQTL_200kb_nominal_result.withFDR.FDR0.1.UCSC_hg19_known_gene.anno_by_chipseeker.genomic_anno_enrichment.result",
       quote=F,sep="\t",col.names=T,row.names=F)

#################barplot
library(ggplot2)
setwd("/data1/gy/ATAC_for_review/FigureS6F/output")
class_colors <- c(
 "3'UTR" = "#EF3B2C",
 "5'UTR" = "#FA9FB5",
 "Distal" = "#41AB5D",
...
p <- ggplot(res, aes(x = Var1)) +  
  ...
  labs(  
    ...
    y = "OR(compared with non-caOCRs)"  
  )
ggsave(p,
       filename="95sample_caQTL_200kb_nominal_result.withFDR.FDR0.1.UCSC_hg19_known_gene.anno_by_chipseeker.genomic_anno_enrichment.barplot.pdf",
       width = 6, height = 5) ##FigureS6F
